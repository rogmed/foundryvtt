...
export class MidiSounds {
	...
	static ActionTypes() {
		...
		let damageEntries = {};
		Object.keys(config.damageTypes).forEach(key => damageEntries[key] = `${i18n(`${systemId}.Damage`)}: ${config.damageTypes[key]}`);
		let itemActionEntries = {};
		Object.keys(config.itemActionTypes).forEach(key => itemActionEntries[key] = `${i18n(`${systemId}.Action`)}: ${config.itemActionTypes[key]}`);
		let actionTypes = {
